...
class TrainLogger:
    ...
    """
    def __init__(self, exp_name_prefix: str = '', logs_dir: PathT = 'logs', make_dir=True):
        """
    ...
    def save_model(self, model: nn.Module, epoch: int, optimizer: Optimizer = None, name='model.pth') -> None:
        """
        Saving a model
        ...
        model_path = os.path.join(self.exp_dir, name)
    ...
    def write_epoch_statistics(self, epoch: int, epoch_time: float, train_loss_pos: float, train_loss_action: float,norm: float,
        ...
        text += 'Gradient norm: %.2f, ' % norm
        text += 'Train loss pos: %.4f, ' % train_loss_pos
        text += 'Train loss action: %.4f, ' % train_loss_action
